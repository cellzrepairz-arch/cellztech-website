...
// A static JSON require is traced by Vercel and avoids import-attribute parser differences.
const copy = createRequire(import.meta.url)('../shared/coupon-copy.json');
...
export async function database(path, { method = 'GET', body, prefer } = {}) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // Only an allowlisted-format provider error code is retained. Never expose
      // a provider message, which can contain customer details or a SQL value.
      if (typeof code === 'string' && /^[A-Z0-9_]{2,20}$/.test(code)) error.providerCode = code;
    } catch { /* Non-JSON provider failures have only the HTTP status. */ }
    throw error;
  }
...
// Read-only diagnostics. The detailed result is only returned to an authenticated
// admin; the public status endpoint continues to expose ready/active booleans only.
export async function couponHealth() {
  const base = {
    checkedAt: new Date().toISOString(),
    emailConfigured: Boolean(process.env.RESEND_API_KEY && process.env.CELLZTECH_FROM_EMAIL)
  };
  const missing = ['SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY'].filter((name) => !process.env[name]);
  if (!process.env.CELLZTECH_PROMO_SIGNING_KEY && !process.env.CELLZTECH_ADMIN_KEY) {
    missing.push('CELLZTECH_PROMO_SIGNING_KEY or CELLZTECH_ADMIN_KEY');
  }
  if (missing.length) return {
    ...base, ready: false, status: 'configuration_missing', missing,
    message: 'The coupon function is missing server settings. Check the listed variable names in this Vercel deployment. Do not share their values.'
  };
  try {
    const health = await rpc('cellztech_promo_ready');
    if (health?.schemaVersion !== VERSION || health?.ready !== true) return {
      ...base, ready: false, status: 'setup_required',
      message: 'Coupon database setup is missing or incomplete. Run supabase/september_coupon.sql in the existing CellzTech Supabase project, then refresh this panel. Do not run the older repair setup SQL.'
    };
    return { ...base, ready: true, status: 'ready', message: 'Coupon database setup is ready. Confirm a signup using your own email before advertising the offer.' };
  } catch (error) {
    const code = error?.providerCode;
    if (error?.status === 401 || error?.status === 403 || code === '42501') return {
      ...base, ready: false, status: 'access_denied',
      message: 'Supabase rejected access to the coupon setup. Confirm the server URL and service-role key belong to the same CellzTech project. The coupon SQL grants the required service-role permissions.'
    };
    if (error?.status === 404 || ['PGRST202', 'PGRST205', '42P01', '42883'].includes(code)) return {
      ...base, ready: false, status: 'setup_required',
      message: 'The coupon database function or table was not found. Run supabase/september_coupon.sql in the existing CellzTech project. If it already succeeded, reload the PostgREST schema cache and refresh this panel.'
    };
    return {
      ...base, ready: false, status: 'database_unavailable',
      message: 'The coupon function could not reach its database check. Verify the CellzTech project is healthy and the server URL is correct, then retry. Do not delete or recreate existing tables.'
    };
  }
}
